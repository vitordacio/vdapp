...
const EmojiSymbol: React.FC<AppProps> = ({ navigation, route }) => {
  const { emojiSymbol, setEmojiSymbol, loadingEmoji } = useEmoji();
  ...
  const fetchNewData = async () => {
    ...
    try {
      animals = await emojiService.findEmojiSymbol({
        page,
      });
      ...
      setEmojiSymbol(prev => [...prev, ...animals]);
      setPage(page + 1);
      setShowLoader(false);
    } catch (error) {
