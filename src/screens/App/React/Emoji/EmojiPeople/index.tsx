...
const EmojiPeople: React.FC<AppProps> = ({ navigation, route }) => {
  const { emojiPeople, setEmojiPeople, loadingEmoji } = useEmoji();
  ...
  const fetchNewData = async () => {
    ...
    try {
      animals = await emojiService.findEmojiPeople({
        page,
      });
      ...
      setEmojiPeople(prev => [...prev, ...animals]);
      setPage(page + 1);
      setShowLoader(false);
    } catch (error) {
