...
const EmojiFace: React.FC<AppProps> = ({ navigation, route }) => {
  const { emojiFace, setEmojiFace, loadingEmoji } = useEmoji();
  ...
  const fetchNewData = async () => {
    ...
    try {
      animals = await emojiService.findEmojiFace({
        page,
      });
      ...
      setEmojiFace(prev => [...prev, ...animals]);
      setPage(page + 1);
      setShowLoader(false);
    } catch (error) {
